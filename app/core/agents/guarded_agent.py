...
class GuardedAgent(Agent):
    ...
    On every user turn, runs ``check_medical_guardrail``.  If a deflection is returned the agent speaks it and raises ``StopResponse`` so the LLM never sees the message.  Otherwise the normal pipeline continues.
    """
